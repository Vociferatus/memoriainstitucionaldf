...
from min_df.contracts import validate_manifest, validate_structured
...
def repeated_marginal_text(
    ...
    for blocks, metadata in zip(pages, page_metadata, strict=True):
...
def enrich_blocks(
    ...
    for blocks, metadata in zip(pages, page_metadata, strict=True):
        ...
        for order, block in enumerate(ordered, start=1):
            ...
            column = (
                0
                if block.width >= page_width * 0.62
                else int((block.x0 + block.x1) / 2 >= split) + 1
            )
...
def structured_document(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "pages": [
            ...
            for metadata, blocks in zip(page_metadata, pages, strict=True)
        ],
    }
