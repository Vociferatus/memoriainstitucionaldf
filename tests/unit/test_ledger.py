...
from min_df.semantic import validate_cnpj
...
def test_v2_migration_encodes_ledger_invariants(project_root: Path) -> None:
    migration = (project_root / "db" / "migrations" / "002_evidence_ledger_v2.sql").read_text(
        encoding="utf-8"
    )
...
def test_canonical_mentions_do_not_repeat_location(project_root: Path) -> None:
    ...
    table = migration.split("CREATE TABLE IF NOT EXISTS evidence_mentions", 1)[1].split(");", 1)[0]
...
def test_cnpj_check_digits() -> None:
    assert validate_cnpj("33.595.865/0001-05")
    assert not validate_cnpj("01.701.830/0001-32")
